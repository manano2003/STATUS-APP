...
export default function HistoryKindergartens() {
  ...
  const [expandedKg, setExpandedKg] = useState<string | null>(null)
  ...
  const daySnapshots = selectedDay
    ? kindergartenHistory.filter(s => s.date === selectedDay)
    : []

  return (
    <PageLayout title="👶 היסטוריית נוכחות בגנים" backTo="/dashboard/history">
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(7, 1fr)', gap: '4px', background: 'var(--color-bg-card)', borderRadius: 'var(--radius)', border: '1px solid var(--color-border)', padding: '12px', marginBottom: '16px' }}>
        ...
        {Array.from({ length: daysInMonth }, (_, i) => {
          ...
          return (
            <button key={day} onClick={() => { if (hasData) { setSelectedDay(isSelected ? null : dateStr); setExpandedKg(null) } }}
        ...
        <>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '8px' }}>
            {kindergartens.map(kg => {
              const snapshot = daySnapshots.find(s => s.kindergartenId === kg.id)
              if (!snapshot) return null
              const isExpanded = expandedKg === kg.id
              return (
                <div key={kg.id}>
                  <button onClick={() => setExpandedKg(isExpanded ? null : kg.id)} style={{
